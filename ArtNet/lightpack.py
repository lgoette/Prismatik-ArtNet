...
# if you want to execute SET-commands, you have to get the control off the Lightpack API Server from the capturing Software with lock
# GET-commands can always be executed
# https://github.com/Atarity/Lightpack-docs/blob/master/EN/Prismatik_API.md#help
	...
	def getColors(self):
		cmd = 'getcolors\n'
		self.connection.send(str.encode(cmd))
		colors = self.__readResult()
		colors = colors.split(':')[1].split(';')
		ret_colors = []
		for color in colors:
			if color.isspace():
				continue
			ret_colors.append(str(int(color.split('-')[0])+1) + '-' + color.split('-')[1])  # LED #, indexed at 1
		return ret_colors
